...
router.get("/normal-groups", async (req, res) => {
  try {
    const id = req.auth?.id;
...
router.get("/group-details/:id", async (req, res) => {
  try {
    ...
    const groupDetails = await Group.findById(groupId).populate([
...
router.post("/group/:groupId/add-member/:member", async (req, res) => {
  try {
